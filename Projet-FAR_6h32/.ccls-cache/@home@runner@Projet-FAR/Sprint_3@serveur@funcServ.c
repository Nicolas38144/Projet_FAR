...
void * sendFile_th(void * list_file_param) {

    /*Accepter une connexion*/
    /*long dSCFile = acceptConnection(dSFileSend);
    char * list_file = (char *)list_file_param;
    printf("Noms des fichiers :\n%s\n", list_file);*/


    /* Récupérer le nom du fichier */
   /* char * fileName = (char *) malloc(sizeof(char)*20);
    receiveMsg(dSCFile, fileName, sizeof(char)*20);
    
    printf("nom sorie : %s\n", fileName);
    printf("taille requete : %ld \n", strlen(fileName));
    if (strcmp(fileName, "error") == 0) {
        printf("L'utilisateur a rentré le mauvais nom de fichier\n");
        free(fileName);
        free(list_file);
        close(dSCFile);
        return NULL;
...
void sendFile(int dSC) {
    ...
    if (strcmp(buffer,"") == 0) {
        ...
        sendPersonnalMsg(dSC, "", 0);
    }
    else {
        printf("Taille du buffer : %ld\n", strlen(buffer));
        sendPersonnalMsg(dSC, buffer, 0);
        pthread_t threadFile;
        int thread = pthread_create(&threadFile, NULL, sendFile_th, (void *)buffer);
        if(thread == -1) {
            perror("Error thread\n");
        }
    }*/

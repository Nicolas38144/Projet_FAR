...
void * receivingFile_th(void * dSFile) {
    ...
    /*har fileServerName[32];
    receiveMsg(dS, fileServerName, sizeof(char)*32);
    fileServerName[strcspn(fileServerName, "\n")] = '\0';
    ...
    if (strlen(fileServerName) == 0) {
        ...
        pathToFile[strcspn(pathToFile, "\n")] = '\0';

        printf("%s %ld\n",pathToFile, sizeof(pathToFile));*/
...
void receivingFile(int dSC) {
    ...
    /*Reception des noms des fichier dans le serveur*/
    /*char * fileNameList = (char *) malloc(sizeof(char)*256);
    receiveMsg(dSC, fileNameList, sizeof(char)*256);
    ...
    if (strlen(fileNameList) == 0) {
        ...
        aS.sin_family = AF_INET;
        inet_pton(AF_INET, arg1, &(aS.sin_addr));
        aS.sin_port = htons(atoi(arg2)+2);*/ /*on passe au port d'après !!!!!!!!!*/
        /* Demander une connexion */
        /*socklen_t lgA = sizeof(struct sockaddr_in);
        int connectR = connect(dSFile, (struct sockaddr *) &aS, lgA);
        if (connectR == -1){
        ...
        printf("dSC : %d\n", dSC);
        printf("dSFile : %d\n", dSFile);
        if (strstr(fileNameList, fileName) == NULL) {
            printf("%s ne fait pas partie des fichiers sur le serveur\n\n", fileName);
            sendMsg(dSFile, "error");
        }
        else {
            printf("avant envoi\n");
            sendMsg(dSFile, fileName);
            printf("après envoi\n");*/
            /*Création du thread pour gérer la reception du fichier*/
            /*pthread_t threadFile;
            int thread = pthread_create(&threadFile, NULL, receivingFile_th, (void *)dSFile);
            if(thread == -1){
                perror("Error thread \n");
            }
        }  
